...
impl HenonMap {
    pub fn encrypt(&mut self, mut img: &DynamicImage, dest_path: &String) -> DynamicImage {
        let (width, height) = img.dimensions();
        let henon_map = self.generate_keystream(width, height, true);
        self.transform_image(img.clone(), dest_path, henon_map)
    }

    pub fn decrypt(&mut self, mut img: &DynamicImage, dest_path: &String) -> DynamicImage {
        let (width, height) = img.dimensions();
    ...
    /// Wrapper
    pub fn transform_image(&mut self, mut img: DynamicImage, dest_path: &String, henon_map: Vec<Vec<u8>>) -> DynamicImage {
